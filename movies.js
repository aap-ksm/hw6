...
window.addEventListener('DOMContentLoaded', async function(event) {
  ...
  let watchedMovieCollection = await db.collection('watched').get()
  let watchedMovies = watchedMovieCollection.docs

  for (let i=0; i < watchedMovies.length; i++){
    let watchedMovieData = watchedMovies[i].data()
    let watchedMovieId = watchedMovies[i].id
    watchedMovieIds.push(watchedMovieId)
  } 
  ...
  async function renderMovie(movieId, moviePoster, movieTitle) {
